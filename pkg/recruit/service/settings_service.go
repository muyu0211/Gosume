...
//
// 分期实现（PRD）：本期仅暴露设置项（GetSettings / SetSettings），
// 条目 CRUD 等 job_process 相关绑定待样本集到位后补齐——前端已有的
// 其余方法调用在绑定缺失时会失败，属于已知未实现，不是缺陷。
type RecruitService struct {
	repo *recruitrepo.SettingsRepo
}
...
// Inject 注入设置存储（复用 gosume.db 共享连接）。
func (s *RecruitService) Inject(repo *recruitrepo.SettingsRepo) {
	s.repo = repo
}
...
func (s *RecruitService) GetSettings() *util.Response {
	return util.DoRsp(util.SuccCode, "", s.repo.Get())
}
