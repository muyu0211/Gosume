...
// ResumeRepo 基于 SQLite 管理简历的持久化。
type ResumeRepo struct {
	db *sql.DB
}

// NewResumeStore 打开或创建 {dataDir}/gosume.db，并初始化表结构。
func NewResumeStore(dataDir string) (*ResumeRepo, error) {
...
func (s *ResumeRepo) DB() *sql.DB {
	return s.db
}

// Close 关闭数据库连接。
func (s *ResumeRepo) Close() error {
	return s.db.Close()
}

// initSchema 在表不存在时创建 resumes 表及其索引。
func (s *ResumeRepo) initSchema() error {
	_, err := s.db.Exec(`
...
// 两者均为空时回退为「未命名简历」。
func (s *ResumeRepo) nameFromResume(resume *model.Resume) string {
	if resume.Meta.Name != "" {
...
func (s *ResumeRepo) Create(resume *model.Resume) (string, error) {
	id := uuid.New().String()
...
// 记录不存在（或已软删除）时返回错误。
func (s *ResumeRepo) Update(id string, resume *model.Resume) error {
	data, err := json.Marshal(resume)
	if err != nil {
		return fmt.Errorf("marshal resume: %w", err)
	}
...
func (s *ResumeRepo) GetByID(id string) (*model.Resume, error) {
	var raw string
	err := s.db.QueryRow(
...
func (s *ResumeRepo) List() ([]ResumeListItem, error) {
	rows, err := s.db.Query(
...
func (s *ResumeRepo) SoftDelete(id string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	result, err := s.db.Exec(
