...
import (
	...
	"fmt"
	"gosume/pkg/log"
	recruit_repo "gosume/pkg/recruit/repo"
	resume_repo "gosume/pkg/resume/repo"
	"os"
	"path/filepath"
)
...
func initResumeStore(dataDir string) *resume_repo.ResumeRepo {
	s, err := resume_repo.NewResumeStore(dataDir)
	if err != nil {
...
// 失败属于不可恢复错误，直接 panic。
func initTemplateStore(resumeStore *resume_repo.ResumeRepo, builtinTemplates embed.FS) *resume_repo.TemplateRepo {
	s, err := resume_repo.NewTemplateStore(resumeStore.DB(), builtinTemplates)
	if err != nil {
...
// initRecruitSettingsRepo 初始化求职进程设置存储，复用简历存储的数据库连接。
// 失败属于不可恢复错误，直接 panic。
func initRecruitSettingsRepo(resumeStore *resume_repo.ResumeRepo) *recruit_repo.SettingsRepo {
	r, err := recruit_repo.NewSettingsRepo(resumeStore.DB())
	if err != nil {
		panic(fmt.Sprintf("Failed to init recruit settings store: %v", err))
	}
	return r
}
...
func initDevWatcher(templateStore *resume_repo.TemplateRepo) chan struct{} {
