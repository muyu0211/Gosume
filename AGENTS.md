...
4. Go 服务（分散在 `pkg/resume/service`、`pkg/recruit/service`、`pkg/setting`、`pkg/autofill/service`、`pkg/tool/service`）实现 Wails `application.Service`，注册为 Wails 服务；前端通过 `frontend/src/services/backend.ts` 的 `callService("ServiceName", "Method", ...args)` 按 `包.结构体.方法` 全名绑定调用。`pkg/ai` 是独立的 AI 调用 SDK（无业务/Wails 依赖），供各 service 使用。
