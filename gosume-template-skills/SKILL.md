...
Gosume 支持的模板包主要包含**两个文件**：

- `template.json`：模板元数据（名称、作者、分类、颜色、特性等）
- `styles.css`：样式表（针对统一 HTML 的固定 DOM 结构写样式）

简历的 HTML 结构**不由模板提供**，而是由应用内置的**统一 HTML**承载。它定义了固定的 DOM 契约（见下文），模板制作者的唯一职责是**写 CSS 决定"简历长什么样"**——布局（单栏/双栏）、配色、字体、间距、头像位置等，全部由 CSS 控制。
...
模板制作者**不写 HTML**，但必须知道统一 HTML 会渲染出什么结构，才能写对 CSS 选择器。契约如下：
...
是否显示头像由模板 CSS 决定（`features.avatar` 仅为元数据，当前版本不参与渲染）。
